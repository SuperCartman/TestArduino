...
#ifdef MOD_UART
{
...
#ifdef MOD_PWR
  // Set up power pin
  MOD_PWR_DDR |= _BV(MOD_PWR_BIT);
  MOD_PWR_PORT |= _BV(MOD_PWR_BIT);
#endif

  // Set up RX mute pin
  MOD_RX_MUTE_DDR |= _BV(MOD_RX_MUTE_BIT);
  MOD_RX_MUTE_PORT &= ~_BV(MOD_RX_MUTE_BIT);
  ...
  // Set up PWM Pin
  MOD_PWM_DDR |= _BV(MOD_PIN_BIT);
  MOD_PORT_PORT &= ~_BV(MOD_PIN_BIT);
  
  // Set Timer in Fast PWM mode
  ocrxa = F_CPU / 250000L - 1L;
  MOD_PWM_TCCRxA = _BV(MOD_PWM_COM) | _BV(WGM20) | _BV(WGM21);  // Fast PWM, non-inverting mode
  MOD_PWM_TCCRxB = _BV(WGM22) | _BV(CS20);                      // Fast PWM, no prescaler
  MOD_PWM_OCRxA = ocrxa;                                        // TOP value
  MOD_PWM_OCRxB = MOD_PWM_OCRxA / 2;   					        // Set duty cycle to 50%
}
#endif
...
# else
  RS485_PORT &= ~_BV(RS485_BIT);
# endif
#elif (MOD_UART==1) // Modulated UART
